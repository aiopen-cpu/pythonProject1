...
if not os.path.exists(USERS_FILE):
    with open(USERS_FILE, "w") as f:
        json.dump({"123": {"password": sha256("123".encode()).hexdigest(), "role": "admin"}}, f)
...
def steamid64_to_steamid(steamid64):
    if steamid64.startswith("STEAM_0:"):
        steamid64 = steamid64.replace("STEAM_0:", "STEAM_1:", 1)
    if steamid64.startswith("STEAM_1:"):
        return steamid64
    try:
...
if not st.session_state["authenticated"]:
    ...
    with tab1:
        ...
        if uploaded_file:
            ...
            selected_supervisor = st.selectbox("Выберите супервизора:", supervisors, key="select_supervisor_stats")
            ...
            # Выводим статистику как список
            st.subheader("Статистика игроков:")
            for stat in player_stats:
                st.write(stat)

            # Формируем отчет
            report_lines = [f"# Статистика за {day_choice} ({today_date})"]
            for _, row in filtered_data.iterrows():
                steam_id = row['FormattedSteamID']
    ...
    with tab2:
        ...
        new_supervisor = st.selectbox("Выберите супервизора:", supervisors, key="add_player_supervisor")
    ...
    with tab3:
        ...
        selected_supervisor = st.selectbox("Выберите супервизора:", supervisors, key="delete_supervisor")
    ...
    with tab4:
        ...
        selected_supervisor = st.selectbox("Выберите супервизора:", supervisors, key="status_supervisor")
        players = {k: v for k, v in steam_id_to_name_supervisor.items() if v["supervisor"] == selected_supervisor}
    ...
    # Вкладка 5: Регистрация супервизора
    if is_admin():
        with tab5:
            st.subheader("Регистрация супервизора")
            supervisor_login = st.text_input("Введите логин для супервизора:", key="register_supervisor_login")
            supervisor_password = st.text_input("Введите пароль для супервизора:", type="password",
                                                key="register_supervisor_password")

            if st.button("Зарегистрировать супервизора", key="register_supervisor_button"):
                if supervisor_login and supervisor_password:
                    hashed_password = sha256(supervisor_password.encode()).hexdigest()
                    users[supervisor_login] = {"password": hashed_password, "role": "supervisor"}
                    save_users()
                    supervisors.append(supervisor_login)
                    save_supervisors()
                    st.success(f"Супервизор {supervisor_login} зарегистрирован.")
                else:
                    st.error("Заполните все поля.")

    # Вкладка 6: Удаление супервизора
    if is_admin():
        with tab6:
            st.subheader("Удаление супервизора")
            selected_supervisor = st.selectbox("Выберите супервизора для удаления:", supervisors,
                                               key="remove_supervisor")
            if st.button("Удалить супервизора", key="remove_supervisor_button"):
                supervisors.remove(selected_supervisor)
                save_supervisors()
                users.pop(selected_supervisor, None)
                save_users()
                st.success(f"Супервизор {selected_supervisor} удалён.")
